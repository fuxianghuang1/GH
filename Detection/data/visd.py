...
import cv2
import numpy as np
from .visd_eval import visd_eval
...
visd_CLASSES = ( '__background__', # always index 0
...
class visdSegmentation(data.Dataset):
...
class AnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=True):
        self.class_to_ind = class_to_ind or dict(
            zip(visd_CLASSES, range(len(visd_CLASSES))))
...
class visdDetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, preproc=None, target_transform=None,
        ...
        for (year, name) in image_sets:
            self._year = year
            rootpath = os.path.join(self.root, 'visd' + year)
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(
            self.root, 'results', 'visd' + self._yearr, 'Main')
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(visd_CLASSES):
            cls_ind = cls_ind 
            if cls == '__background__':
                continue
            print('Writing {} visd results file'.format(cls))
    ...
    def _do_python_eval(self, output_dir='output'):
        rootpath = os.path.join(self.root, 'visd' + self._year)
        ...
        if output_dir is not None and not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        for i, cls in enumerate(visd_CLASSES):

            if cls == '__background__':
                continue

            filename = self._get_voc_results_file_template().format(cls)
            rec, prec, ap = visd_eval(
