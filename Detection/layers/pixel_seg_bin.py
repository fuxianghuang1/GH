    ...
    for idx in range(n):
        ...
        if areas1.shape==torch.Size([]):
            ...
            for i in areas1:
                ...
                score_label[idx,int(coor[0][bareas[1][i][0]]):int(coor[1][bareas[1][i][0]]+1),int(coor[2][bareas[1][i][0]]):int(coor[3][bareas[1][i][0]]+1)]=1
    #t1=time.time()
    #print('told=%.4f'%(t1-t0))
    #print(dddd)
    return score_label
...
def downsample_bin(input_image,targets,use_gpus,th1,th2,width,height,steps):
