...
from .base_models import vgg, vgg_base, vgg_base_prune, vgg_prune


class BasicConv(nn.Module):

    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, relu=True,
                 bn=True, bias=False, up_size=0):
...
def add_extras(cfg, i, batch_norm=False):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':
                layers += [BasicConvBN(in_channels, cfg[k + 1],
                                     kernel_size=(1, 3)[flag], stride=2, padding=1)]
            else:
                layers += [BasicConvBN(in_channels, v, kernel_size=(1, 3)[flag])]
            flag = not flag
        in_channels = v
    return layers
...
def feature_transform_module(vgg, extral, size):
    ...
    layers = []
    # conv4_3
    # layers += [BasicConv(vgg[21].out_channels, 256, kernel_size=1, padding=0)]
    # # fc_7
    ...
    # prune
    layers += [BasicConv(vgg[21].out_channels, 135, kernel_size=1, padding=0)]
    # # fc_7
    layers += [BasicConv(vgg[-2].out_channels, 29, kernel_size=1, padding=0, up_size=up_size)]
    layers += [BasicConv(extral[-1].out_channels, 15, kernel_size=1, padding=0, up_size=up_size)]
...
def pyramid_feature_extractor(size):
    if size == 300:
        layers = [#BasicConvBN(256 * 3, 512, kernel_size=3, stride=1, padding=1),
                  BasicConvBN(179, 512, kernel_size=3, stride=1, padding=1), # prune
...
def build_net(size=300, num_classes=21):
    ...
    return FSSD(*feature_transform_module(vgg_prune(vgg_base_prune[str(size)], 3), add_extras(extras[str(size)], 1024), size=size),
