...
class resnet_seg_pixel(nn.Module):
    def __init__(self, phase,extras,size,block, layers, num_classes=1000):
        super(resnet_seg_pixel, self).__init__()
        self.phase = phase
        ...
        self.deconv1=nn.ModuleList([BasicdeConv(512, 512, kernel_size=3, stride=1, padding=0, relu=False,bn=False),
                                    BasicdeConv(512, 1024, kernel_size=3, stride=1, padding=0, relu=False,bn=False),
...
def build_net(phase, size=320, num_classes=21):
    ...
    return resnet_seg_pixel(phase,extras_,size,block,[3,4,6,3],num_classes=num_classes)
