...
class CSSDataset(BaseDataset):
  ...
  #{'make_new_color': 'green', 'from': [798, 903, 581], 'str': ['make'], 'to': [1000, 1001, 1002],
  #'target_shape': 'circle', 'target_size': 'large', 'to_str': 'make middle-left large circle green', 
  #'target_pos': 'middle-left', 'make_new_size': None, 'target_color': None, 'type': 'make'}
...
class MITStates(BaseDataset):
  ...
  def __getitem__(self, idx):
    ...
    mod_str = 'replace '+ self.imgs[idx]['adj'] + ' with ' + self.imgs[target_idx]['adj']
  ...
  def caption_index_sample_(self, idx):
    ...
    target_adj = random.choice(self.noun2adjs[noun])#choice adj 
    target_caption = target_adj + ' ' + noun
    target_idx = random.choice(self.caption2imgids[target_caption])# choice idx from which has the same capthion
