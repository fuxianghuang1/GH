...
def image_classification_test(loader, model, gvbg=1):
    start_test = True
    with torch.no_grad():
...
def train(config):
    ...
    for i in range(config["num_iterations"]):
        ...
        ################ GH or GH++ ################
        if config["GH"] == True:
            # transfer_loss and classifier_loss
            optimizer.zero_grad()
            transfer_loss.backward(retain_graph=True)
            grad1 = getgrad(base_network)
            optimizer.zero_grad()
            classifier_loss.backward(retain_graph=True)
            grad2 = getgrad(base_network)
            optimizer.zero_grad()
            ...
            # gvbg and gvbd  
            optimizer.zero_grad()
            gvbg.backward(retain_graph=True)
            grad1 = getgrad(base_network)
            optimizer.zero_grad()
            gvbd.backward(retain_graph=True)
            grad2 = getgrad(base_network)
            optimizer.zero_grad()
            if torch.dot(grad1,grad2)<0:
                gh1_gd = 1 - (torch.dot(grad1,grad2))/(torch.dot(grad1,grad1))
                gh2_gd = 1 - (torch.dot(grad1,grad2))/(torch.dot(grad2,grad2))
            else:
                gh1_gd = 1
                gh2_gd = 1
            total_loss = loss_params["trade_off"] * gh1 * transfer_loss + gh2 * classifier_loss + config["GVBG"] * gh1_gd * gvbg + abs(config['GVBD']) * gh2_gd * gvbd

        elif config["GH_new"] == True:
        # transfer_loss and classifier_loss
            optimizer.zero_grad()
            transfer_loss.backward(retain_graph=True)
            grad1 = getgrad(base_network)
            ...
            if dot_product<0:            
                ...
                beta = config["trade_off_1"] * beta_max
            ...
            if dot_product<0:            
                ...
                beta_max = angle_degrees - 90
                beta = config["trade_off_1"] * beta_max
                gh1_gd = 1 + 2 * math.sin(math.radians(0.5 * beta))
                gh2_gd = 1 + 2 * math.sin(math.radians(0.5 *(beta + 90 - theta)))
            else:
                gh1_gd = 1
                gh2_gd = 1                     
            total_loss = loss_params["trade_off"] * gh1 * transfer_loss + gh2 * classifier_loss + config["GVBG"] * gh1_gd * gvbg + abs(config['GVBD']) * gh2_gd * gvbd
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_iterations', type=int, default=10000, help="interation num ")
    ...
    parser.add_argument('--GVBD', type=float, default=1, help="mu: parameter for GVBD (if mu==0 then GVBD is not utilized)")
    ...
    parser.add_argument('--GH_new', type=bool, default=False, help="utilize GH++ or not)")
    parser.add_argument('--trade_off_1', type=float, default=0.5, help="Harmonic range in [0,1]")
    ...
    config["GH"] = args.GH
    config["GH_new"] = args.GH_new
    config["trade_off_1"] = args.trade_off_1
