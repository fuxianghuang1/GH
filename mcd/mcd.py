...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        y2_s, y2_t = y_2.chunk(2, dim=0)
        y1_t, y2_t = F.softmax(y1_t, dim=1), F.softmax(y2_t, dim=1)
        loss_c = F.cross_entropy(y1_s, labels_s) + F.cross_entropy(y2_s, labels_s) + \
               (entropy(y1_t) + entropy(y2_t)) * args.trade_off_entropy
        loss_t = - classifier_discrepancy(y1_t, y2_t) * args.trade_off
        ...
        if args.GH == True:
            ...
            if torch.dot(grad1, grad2) < 0:
                ...
                gh1 = 1
                gh2 = 1
            total_loss = gh1 * loss_c + gh2 * loss_t
...
if __name__ == '__main__':
    ...
    parser.add_argument('--GH', type=bool, default=True,
