...
def evaluate(device, model_instance, input_loader):
...
def evaluate_all(device, model_instance, dataloaders, iter_num, args):
    flag = args.dataset=='visda'
    if args.eval_source:
        eval_result = evaluate(device, model_instance, dataloaders["source_val"])
    ...
    if args.eval_target and dataloaders["target_val"] is not None:
        eval_result = evaluate(device, model_instance, dataloaders["target_val"])
        if args.use_tensorboard:
    ...
    if args.eval_test and dataloaders["test"] is not None:
        if type(dataloaders["test"]) is list or type(dataloaders["test"]) is tuple:
            for i, t_test_loader in enumerate(dataloaders["test"]):
                eval_result = evaluate(device, model_instance, t_test_loader)
            ...
            eval_result = evaluate(device, model_instance, dataloaders["test"])
            if args.use_tensorboard:
