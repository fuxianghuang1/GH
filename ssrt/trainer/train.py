...
def train_source(device, model_instance, dataloaders, optimizer, lr_scheduler, args):
    ...
    for epoch in range(args.train_epoch):
        ...
        if (epoch+1) % args.eval_epoch == 0:
            evaluate_all(device, model_instance, dataloaders, epoch+1, args)
...
def train(device, model_instance, dataloaders, optimizer, lr_scheduler, args):
    model_instance.set_train(True)
    base_net = model_instance.net.base_network
    ...
    for epoch in range(args.train_epoch):
        for _ in tqdm.tqdm(
            ...
            if args.rand_aug:
                inputs_rand_target = inputs_rand_target.to(device)
            ...
            optimizer.zero_grad()
            if args.rand_aug:
                total_loss = model_instance.get_loss(optimizer, base_net, inputs_source, inputs_target, labels_source, labels_target,
...
def train_source_main(args_, header=''):
    ...
    try:
        ...
        model_instance = Model(device=device, base_net=args.base_net, bottleneck_dim=args.bottleneck_dim, use_gpu=True, class_num=args.class_num, args=args)
    except:
    ...
    train_source(device, model_instance, dataloaders, optimizer=optimizer, lr_scheduler=lr_scheduler, args=args)
...
def train_main(args_, header=''):
    ...
    train(device, model_instance, dataloaders, optimizer=optimizer, lr_scheduler=lr_scheduler, args=args)